                    ...
                    <form action="{{ route('booking.train_bank_account', ['id' => $booking->id]) }}" method="GET">
                		...
                		<div class="row bg-default" style="border-top: 1px solid #ddd; border-bottom: 1px solid #ddd; margin-bottom: 10px; color: #777">
                			...
                			<div class="col-md-12 booking-price-list">
                				{{ $booking->bookingDetail->trainJourney->trainRoute->train->name }} (Dewasa) x{{ $booking->bookingDetail->adult_number }}
                				<span class="pull-right">Rp {{ $booking->bookingDetail->adult_fare }}</span>
                			</div>
                			@endif
                			@if ($booking->bookingDetail->baby_number != 0)
                			<div class="col-md-12 booking-price-list">
                				{{ $booking->bookingDetail->trainJourney->trainRoute->train->name }} (Bayi) x{{ $booking->bookingDetail->baby_number }}
    						...
    						<div class="col-md-12 booking-price-list" style="padding-top: 25px">
    							Harga Total <strong class="pull-right booking-price-total">Rp {{ $booking->bookingDetail->train_fare_total }}</strong>
    						</div>
                		</div>
                		<div class="row">
    	            		<div class="col-md-12">
